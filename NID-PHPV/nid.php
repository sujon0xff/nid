...
$baba = isset($_GET['baba']) ? htmlspecialchars($_GET['baba']) : 'ধনল';
$mom = isset($_GET['mom']) ? htmlspecialchars($_GET['mom']) : 'শরিফা বেগম';
$birth = isset($_GET['birth']) ? htmlspecialchars($_GET['birth']) : '10 oct 2000';
$nid = isset($_GET['nid']) ? htmlspecialchars($_GET['nid']) : '8818181019168';
$hold = isset($_GET['hold']) ? htmlspecialchars($_GET['hold']) : '300';
$address = isset($_GET['address']) ? htmlspecialchars($_GET['address']) : 'বরিশাল, পারা, ধনরা';
$grade = isset($_GET['grade']) ? htmlspecialchars($_GET['grade']) : 'A+';
...
function overlayImageFromURL($template_image_path, $uploaded_image_url, $name, $name_en, $baba, $mom, $birth, $nid, $hold, $address, $grade, $dakgor, $signature) {
    ...
    addTextToImage($template, $grade, 31, 410, 1112, $red, $font_path);
...
// ID কার্ড তৈরি করা
overlayImageFromURL($template_image, $image_url, $name, $name_en, $baba, $mom, $birth, $nid, $hold, $address, $grade, $dakgor, $signature);
?>
